...
class StorageManager {
    ...
    func save(_ taskTitle: String, completion: (Task) -> Void) {
        write {
            let taskList = Task(value: [taskTitle])
            realm.add(taskList)
            completion(taskList)
        }
    }
    ...
    func edit(_ taskTitle: Task, taskNote: Task, newTitle: String, newNote: String) {
        write {
            taskTitle.title = newTitle
            taskNote.note = newNote
        }
    }
